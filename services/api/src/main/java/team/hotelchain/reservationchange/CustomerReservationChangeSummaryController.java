...
public class CustomerReservationChangeSummaryController {
    ...
    public ResponseEntity<Summary> get(@PathVariable UUID id, @RequestHeader("X-Reservation-Token") String token) {
        ...
        Summary summary = jdbc.query("""
                select r.id request_id, r.reservation_id, r.request_origin, r.status, r.target_check_in, r.target_check_out,
                ...
                """, rs -> rs.next() ? new Summary(rs.getObject("request_id", UUID.class), rs.getObject("reservation_id", UUID.class), rs.getString("request_origin"), rs.getString("status"),
    ...
    public record Summary(UUID requestId, UUID reservationId, String requestOrigin, String status, LocalDate checkIn, LocalDate checkOut,
