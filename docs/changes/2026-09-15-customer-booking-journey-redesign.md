...
실제 카드·OTP·계정 인증은 사용자가 직접 수행한다. 분기 최종 수정 당시에는 최초 승인만 지원했으나, 메인 병합에서 기존 Toss 추가 결제·부분/전체 환불 어댑터를 보존했다. `PAYMENT_PROVIDER`와 `RESERVATION_CHANGE_GATEWAY`를 거래 공급자와 맞춰야 하며 혼합·비활성 공급자 정산은 차단한다. 실제 추가 결제·부분 환불과 이번 병합 후 UI는 미검증이다.
