...
검증은 로컬 테스트 DB(`hotel_chain_test`)와 Playwright API 모형만 사용했다. 테스트 token·staff·문서는 검증용이며 실제 사용자 데이터로 요청하지 않았다.
...
로컬 관리자에서 저장된 초안을 선택하고 **실제 화면 미리보기**로 배너·만료·공개본 복귀를 검토한다. 운영 반영 전에는 HTTPS/프록시 설정을 배포 환경에서 확인한다.
