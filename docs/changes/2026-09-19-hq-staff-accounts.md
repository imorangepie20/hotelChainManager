...
- `admin-menu-roadmap.md` 3번 `직원 계정 관리`가 다음 대상이었다. `StaffDevAccountInitializer`가 환경 변수 비밀번호로 개발 계정을 초기화할 뿐, 애플리케이션에서 직원을 만들거나 조회할 수단이 없었다.
- 전체 범위(생성·역할 변경·지점 할당·비밀번호 초기화·비활성) 중 **생성과 조회**만 먼저 했다. 비밀번호 재발급과 비활성은 권한이 넓어지므로 다음 작업으로 뺐다.
- 임시 비밀번호를 서버가 만들어 한 번만 내보내는 동선으로, 비밀번호 원문을 저장하지 않는 기존 원칙을 유지했다.
...
- `staff/StaffAccountCommandService`: `create(token, idempotencyKey, request)`.
  - `requireHeadquarters`로 본사만 허용하고, BCrypt로 비밀번호를 해싱한다.
  - 임시 비밀번호는 16자 무작지(`IO01l` 같은 혼동 문자 제외)로 생성해 응답에 한 번만 내보낸다.
  - 이메일 중복은 삽입 전에 확인해 `StaffEmailConflictException`으로 409를 낸다. 삽입 실패 뒤 재조회하면 PostgreSQL 트랜잭션이 중단되기 때문이다.
  - 멱원은 `staff_account_command`의 `(staff_id, idempotency_key)` UNIQUE로 막는다.
- `staff/StaffAccountController`: `GET /api/staff/staff`, `POST /api/staff/staff`. `@ExceptionHandler`로 이메일 중복을 409 `STAFF_EMAIL_DUPLICATE`로 매핑한다.
- `staff/StaffAccountCreateRequest`: 이메일 형식·254자, 이름 1~100자, 역할 4종, `BRANCH_STAFF`는 지점 필수·본사 역할은 지점 없음을 검증한다.
...
- `nav.ts`에 `본사 관리` 그룹을 추가하고 `직원 권한`을 넣었다. `HQ_ADMIN`에만 노출되고 지점 직원은 메뉴·명령 팔레트에서 보지 못한다.
...
- 라이브: API 재빌드 뒤 V47 `success=t`, health `UP`. 본사 세션으로 목록 200, 생성 201, 멱원 재호출 200 같은 ID, 이메일 중복 409 `STAFF_EMAIL_DUPLICATE`, 형식·역할·지점 누락·본사 역할+지정 400, 없는 지점 404, 멱원 키 누락 400, 지점 직원 조회·생성 403, 세션 없음 401을 확인했다. 발급된 임시 비밀번호로 실제 로그인 201을 확인했다. 관리자 4001 브라우저에서 본사 로그인 뒤 `직원 6명` → 추가 → `7명` 전환과 임시 비밀번호 표시를 확인했다.
- 검증용 직원과 멱원 기록·세션을 모두 삭제해 원래 6명으로 복원했다. `staff_account_command` 행 수 0.

## 미검증 항목

- 비밀번호 재발급. 임시 비밀번호 1회 발급만 있고 재발급 동선은 없다.
...
- 동시 요청의 실제 경합(두 본사 관리자가 동시에 같은 이메일을 보낼 때 한 쪽만 201). 이메일 중복 사전 확인과 UNIQUE 제약으로 막지만 부하 상황에서의 측정은 하지 않았다.
