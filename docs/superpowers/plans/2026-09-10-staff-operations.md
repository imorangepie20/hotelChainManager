...
**Goal:** 실제 객실 배정부터 체크아웃·청소 완료·노쇼 처리까지의 서버 운영 흐름을 구현한다.
...
- [x] 조회 API 결과를 관리자 화면에 표시한다.
- [x] 지점 직원 UI에서 배정·체크인·체크아웃·청소 완료를 실행한다.
- [ ] UI 접근성·모바일 동작을 확인한다.

## Task 3: 기록 갱신

- [x] 노쇼 처리를 추가하고 구현 범위·검증·남은 객실 변경 기능을 기록한다.

## 후속: 객실 배정 UI 기반

- [x] 예약의 지점·객실 유형·청소 상태·숙박 기간을 만족하는 배정 후보 API를 추가한다.
- [x] 당일 도착 목록에서 후보 실제 객실 번호를 선택해 배정한다.
