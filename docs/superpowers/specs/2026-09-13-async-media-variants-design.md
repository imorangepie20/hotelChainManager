...
1. 전용 단일 스레드 scheduler가 2초 간격으로 작업 하나를 요청한다. 기본 scheduler의 예약 만료 작업과 분리해 인스턴스당 미디어 변환은 동시에 하나만 실행한다.
2. claim transaction은 만료된 PROCESSING attempt 3을 `FOR UPDATE SKIP LOCKED LIMIT 1`로 한 건만 FAILED 처리한 뒤, 같은 잠금 방식으로 PENDING, 재시도 시각이 지난 FAILED, lease가 만료된 PROCESSING 중 시도 횟수가 3 미만인 작업 하나를 선택한다. 잠긴 terminal row는 다른 작업 선점을 막지 않는다.
...
6. 현재 attempt·claim token을 row lock 아래에서 확인하고 임시 파일을 claim별 UUID가 포함된 고유 immutable key로 이동한 뒤 READY와 결과 메타데이터를 기록한다. 기존 파일은 덮어쓰지 않는다.
...
dialog가 열려 있고 선택 자산이 ACTIVE이며 PENDING/PROCESSING 또는 FAILED 1·2회 variant가 있을 때만 응답 완료 2초 후 catalog를 다시 읽는다. 자동 재시도 backoff도 polling을 유지하며 모든 variant가 READY/FAILED 3회이거나 자산이 보관됐거나 dialog가 닫히면 중단한다. polling과 수동 재시도 응답에서는 variants만 병합해 입력 중 메타데이터와 그 expectedVersion을 유지한다. 기존 업로드·메타데이터·보관·교체 interaction은 유지한다.
...
- 새 관리자는 자산을 반환하는 모든 미디어 API 경계에서 이전 API의 누락 `variants`를 빈 배열로 정규화한다. 현재 `WebsiteMediaAsset`의 필수 배열 계약은 유지한다.
...
- 전용 단일 스레드 미디어 scheduler는 실행당 한 작업만 처리해 CPU·메모리 급증을 제한한다. 예약 만료는 기본 scheduler에서 계속 실행한다. 업로드 제한 24 MP를 그대로 사용한다.
...
- 관리자 Playwright에서 네 상태 표시, FAILED 1·2회 backoff polling, terminal/보관 상태 polling 중단, 재시도 중 메타데이터 버전 보존과 구버전 API 카탈로그 호환성을 확인한다.
- 실제 PostgreSQL에서 잠긴 만료 attempt 3 row를 건너뛰어 다른 작업을 선점하며, 한 번에 한 terminal row만 정리하는지 확인한다. 실제 Spring 스케줄 등록에서 미디어 인코더를 막아도 예약 만료가 실행되는지 확인한다.
