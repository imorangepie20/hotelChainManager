...
- 고객 웹 전체 회귀의 기존 실패 5건을 재현했다. 4건은 필수 전화번호·접근성 셀렉터·예약 변경 폴링·결제 버튼 수명주기를 현재 계약에 맞게 고쳤고, 실제 결함 1건은 결제 모드와 Toss 키 환경 불일치였다.
- `toss-test`는 `test_`, `toss-live`는 `live_` 키만 SDK 로드 전에 허용한다. 신규 예약과 예약 변경 결제 모두 같은 검사를 사용한다.
- 고객 웹 단위 계약, 대상 5건, 전체 Playwright 34건과 프로덕션 빌드가 통과했다.
- 호텔·객실 및 재고·가격 쓰기 기능은 관리자 Chromium 61건과 Spring 통합 206건으로 대조했고 로드맵 누락이 없었다. 상세 기록은 [변경 기록](../changes/2026-09-27-customer-playwright-baseline-and-catalog-write-audit.md)을 따른다.

## CMS 콘텐츠 페이지 선택기 표시명 수정 (2026-09-26)

- `콘텐츠 페이지 만들기`에서 `BRAND`와 상위 섹션 UUID가 보이던 원인은 Base UI Select에 값-표시명 목록을 전달하지 않은 것이었다.
- 콘텐츠 유형·상위 섹션·소유 지점·객실 유형에 표시명 매핑을 추가했다. 생성 API에는 기존 enum과 UUID를 그대로 보내므로 서버 계약은 바뀌지 않는다.
- 관리자 CMS Chromium 62건과 TypeScript 검사가 통과했고 ESLint는 오류 0건이다. 상세 기록은 [CMS 콘텐츠 페이지 선택기 표시명 수정](../changes/2026-09-26-cms-select-display-label.md)을 따른다.
