...
- 관리자에 `예약 관리` 화면을 추가해 지점 선택, 날짜 달력, 검색, 상태 필터, 반응형 예약 표와 읽기 전용 상세 대화상자를 한 화면에서 제공한다. 예약 변경·취소와 당일 운영 액션은 이번 범위에 포함하지 않았다.
...
1. 예약 변경·취소를 직원 기능으로 열기 전에 변경 가능 상태, 재고 복구, 환불, 감사 이력과 역할별 승인 범위를 설계한다.
