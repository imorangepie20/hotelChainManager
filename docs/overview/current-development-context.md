...
- V21은 영어 번역 초안을 version에 묶어 `DRAFT → IN_REVIEW → APPROVED → PUBLISHED`로 검토·승인한 뒤에만 발행하도록 제한하고, V22는 비-DRAFT 상태의 검토 version `NOT NULL`을 DB에서 강제한다. 저장은 검토·승인을 무효화하되 기존 공개본과 공개 media usage를 유지하고, 반려는 필수 사유와 함께 초안으로 되돌린다. 일반 페이지 보관은 공개를 중단하며 `PUBLISHED`를 같은 version의 `APPROVED`로 바꾼다. 관리자 홈·지점·일반 페이지는 공통 action bar, 최근 검토 event 50건, 기존 발행 이력, mutation 성공 뒤 이력 읽기 재시도를 제공하며 review mutation 중 sibling 편집도 차단한다. PostgreSQL 통합 16건과 관리자 편집기 Chromium E2E 46건, 관리자·고객 production build를 통과했다. Docker API 재빌드에서 개발 DB V20→V21 적용과 health `UP`을 확인했고, 실제 영어 공개 7개 경로가 모두 HTTP 200·비어 있지 않은 콘텐츠를 반환했다. 읽기 전용 DB 집계는 발행 source와 초안 version이 같은 7행 모두 `PUBLISHED`, 불일치 0행이다. 후속 V22·동시 편집 보강은 focused migration 1건, pending-review E2E 1건, TypeScript 검사로 확인했으며 전체 회귀는 반복하지 않았다. 물리 장치, 역할 분리, 예약 발행, 알림·경보, 한국어 승인은 범위 밖이다. 상세 결과와 이전 binary 롤백 제약은 [영어 번역 검토·승인 변경 기록](../changes/2026-09-12-cms-translation-review.md)을 따른다.
