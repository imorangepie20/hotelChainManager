...
터미널에서 아래처럼 호출한다. `.cmd` 파일은 PowerShell 실행 정책을 현재 실행에만 우회하므로 별도 설정이 필요 없다.
...
스크립트는 자신이 시작한 고객·관리자 프로세스와 Docker 컨테이너만 정지한다. 이미 실행 중이던 서버는 그대로 둔다. `.ps1`을 직접 실행해 실행 정책 오류가 나면 현재 창에서만 아래 명령을 한 번 실행한 뒤 다시 시도한다.
