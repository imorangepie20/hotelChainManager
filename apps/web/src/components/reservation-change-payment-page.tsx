...
import { customerPaymentFailureStatus, changePaymentStatusForDisplay, canRecoverChangePayment, describeReservationChangePayment, shouldPollReservationChangePayment } from '../lib/reservation-change-payment-state'
...
export function ReservationChangePaymentPage({ locale = 'ko', returned }: Props) {
  ...
  useEffect(() => {
    ...
    if (!initialRequest.current) initialRequest.current = (async () => {
      ...
      if (modes.changeProvider === 'toss-test') {
        ...
        else if (returnedFromToss || sessionStorage.getItem('tossChangeStarted')) toss = await api.tossChangeStatus()
      }
      return { current: await api.reservationChangePayment(), provider: modes.changeProvider, toss }
    })()
    initialRequest.current.then(result => {
      ...
      if (active) {
        if (cause instanceof ApiFailure && customerPaymentFailureStatus(cause.status)) setSessionExpired(true)
  ...
  async function checkout() {
    ...
    try {
      ...
      if (cause instanceof ApiFailure && customerPaymentFailureStatus(cause.status)) { setPayment(previous => previous ? { ...previous, status: 'EXPIRED' } : previous); setSessionExpired(true) }
      else setError(text.failure)
