...
import { customerPaymentFailureMessage, customerPaymentFailureStatus, changePaymentStatusForDisplay, canRecoverChangePayment, describeReservationChangePayment, shouldPollReservationChangePayment } from '../lib/reservation-change-payment-state'

import { localizedTossCheckout, requestTossCheckout, type TossReturn, type TossStatus } from '../lib/toss-payments'
...
export function ReservationChangePaymentPage({ locale = 'ko', returned }: Props) {
  ...
  useEffect(() => { if (error || sessionExpired) alert.current?.focus() }, [error, sessionExpired])
  const modeRequest = useRef<ReturnType<typeof api.paymentModes> | null>(null)
  const initialRequest = useRef<Promise<{ current: ReservationChangePayment; provider: PaymentProvider; toss: TossStatus | null }> | null>(null)
  const returnedFromToss = returned.kind !== 'none'
  useEffect(() => {
    let active = true
    const modesRequest = modeRequest.current ??= api.paymentModes()
    // Keep the configured provider even if confirmation or detail loading fails.
    void modesRequest.then(modes => { if (active) setProvider(modes.changeProvider) }).catch(() => {})
    if (!initialRequest.current) initialRequest.current = (async () => {
      const modes = await modesRequest
      ...
      if (paymentActions(modes.changeProvider).toss) {
        if (returned.kind === 'success') toss = await api.tossChangeConfirm(returned.input)
          ...
          try { toss = await api.tossChangeStatus() }
          catch { sessionStorage.removeItem('tossChangeStarted') }
        }
      }
      return { current, provider: modes.changeProvider, toss }
    })()
    initialRequest.current.then(result => {
      if (active) { setPayment(result.current); setProvider(result.provider); setTossState(result.toss); setSessionExpired(false) }
    }).catch(cause => {
      if (active) {
        if (cause instanceof ApiFailure && customerPaymentFailureStatus(cause.status)) { setPayment(null); setSessionExpired(true) }
        else setError(text.failure)
      }
    }).finally(() => { if (active) setLoading(false) })
    return () => { active = false }
  }, [token, returned, returnedFromToss, text.failure])
  useEffect(() => {
    ...
    async function refresh() {
      ...
      catch (cause) { if (active) { if (cause instanceof ApiFailure && customerPaymentFailureStatus(cause.status)) { setPayment(null); setSessionExpired(true); setError('') } else setError(cause instanceof ApiFailure ? cause.message : text.failure) } }
  ...
  async function refreshToss() {
    ...
    busyRef.current = true; setBusy(true); setError('')
    try {
      const [statusResult, currentResult, modesResult] = await Promise.allSettled([
        api.tossChangeStatus(), api.reservationChangePayment(), api.paymentModes(),
      ])
      if (modesResult.status === 'fulfilled') setProvider(modesResult.value.changeProvider)
      if (statusResult.status === 'fulfilled') setTossState(statusResult.value)
      if (currentResult.status === 'fulfilled') { setPayment(currentResult.value); setSessionExpired(false) }
      const failed = [statusResult, currentResult].find(result => result.status === 'rejected')
      if (failed?.status === 'rejected') {
        const cause: unknown = failed.reason
        if (cause instanceof ApiFailure && customerPaymentFailureStatus(cause.status)) { setPayment(null); setSessionExpired(true) }
        else setError(text.failure)
      }
    } catch { setError(text.failure) }
    finally { busyRef.current = false; setBusy(false) }
  }
  const canStartPayment = !returnedFromToss && (!tossState || tossState.paymentStatus === 'NEW') && (paymentActions(provider).fake || paymentActions(provider).toss)
  const display = payment ? describeReservationChangePayment(payment) : null
  const statusText = display ? paymentStatusText(changePaymentStatusForDisplay(display.kind, tossState?.paymentStatus), locale) : ''
  const amountLabel = display ? paymentAmountLabel(display.direction, locale) : ''
  return <main className="change-payment-shell"><section className="change-payment-card" aria-labelledby="change-payment-title"><header className="change-payment-header"><span className="change-payment-mark" aria-hidden="true"><ShieldCheck size={24} /></span><div><p>{text.kicker}</p><h1 id="change-payment-title">{text.title}</h1></div></header>{loading && <div className="change-payment-state" role="status">{text.loading}</div>}{sessionExpired && !payment && <div className="change-payment-state" role="alert" ref={alert} tabIndex={-1}>{paymentStatusText('EXPIRED', locale)}</div>}{error && <div className="change-payment-state is-error" role="alert" ref={alert} tabIndex={-1}>{error}</div>}{!loading && !sessionExpired && !display?.canCheckout && canRecoverChangePayment(returnedFromToss, provider, tossState !== null, display?.kind) && <button className="outline" type="button" disabled={busy || loading} onClick={() => void refreshToss()}>{locale === 'ko' ? '서버 상태 다시 확인' : 'Check server status'}</button>}{!loading && payment && <><div className={`change-payment-status ${display?.kind === 'COMPLETED' ? 'is-complete' : ''}`} role="status"><strong>{statusText}</strong><span>{locale === 'en' && payment.environmentLabel === '테스트 결제' ? 'Test payment' : payment.environmentLabel}</span></div>{display?.kind === 'CANCELLED' && <p className="change-payment-state is-error" role="status">{locale === 'ko' ? '결제가 실패했거나 취소되었습니다. 호텔 직원에게 새 결제 링크를 요청해 주세요.' : 'The payment failed or was cancelled. Please ask hotel staff for a new payment link.'}</p>}<dl className="change-payment-details"><div><dt>{text.number}</dt><dd>•••• {payment.reservationNumberSuffix}</dd></div><div><dt aria-label={text.before}>{text.before}</dt><dd>{formatDate(payment.previousCheckIn, locale)} – {formatDate(payment.previousCheckOut, locale)}<span>{payment.previousRoomTypeName} · {payment.previousRatePlanName}</span><strong>{formatMoney(payment.previousTotalKrw, payment.currency, locale)}</strong></dd></div><div className="is-changed"><dt aria-label={text.after}>{text.after}</dt><dd>{formatDate(payment.checkIn, locale)} – {formatDate(payment.checkOut, locale)}<span>{payment.roomTypeName} · {payment.ratePlanName}</span><strong>{formatMoney(payment.totalKrw, payment.currency, locale)}</strong></dd></div><div><dt><Clock3 size={17} aria-hidden="true" /> {text.expires}</dt><dd>{formatDate(payment.expiresAt, locale, true)}</dd></div></dl><div className="change-payment-total"><span>{amountLabel}</span><strong>{amountLabel} {formatPaymentAmount(Math.abs(payment.differenceKrw), payment.currency, locale)}</strong></div>{display?.canCheckout && canStartPayment && <button className="change-payment-action" type="button" onClick={checkout} disabled={busy}><CreditCard size={19} aria-hidden="true" />{busy ? text.preparing : provider === 'toss-test' ? (locale === 'ko' ? '토스 테스트 결제' : 'Toss test payment') : `${formatPaymentAmount(payment.additionalAmountKrw, payment.currency, locale)} ${text.pay}`}</button>}{(display?.kind === 'COMPLETED' || display?.kind === 'REFUND_PENDING') && <div className="booking-result-actions"><a className="primary" href={`${locale === 'en' ? '/en' : ''}/reservations/${payment.reservationId}`}>{locale === 'ko' ? '예약 상세 보기' : 'View reservation'}</a><a className="outline" href={locale === 'en' ? '/en' : '/'}>{locale === 'ko' ? '홈으로' : 'Home'}</a></div>}<p className="change-payment-note">{text.note}</p></>}</section></main>

}

function paymentStatusText(status: string, locale: 'ko' | 'en') {
  const labels = locale === 'ko'
    ? { AWAITING_PAYMENT: '추가 결제 대기 중', READY_TO_APPLY: '결제 확인 완료', APPLYING: '예약 변경을 반영 중입니다', COMPLETED: '예약 변경이 완료되었습니다', CANCELLED: '결제가 취소되었습니다', EXPIRED: customerPaymentFailureMessage(locale), RECONCILIATION_REQUIRED: '결제 결과를 확인 중입니다', REFUND_PENDING: '차액 환불을 처리 중입니다', UNKNOWN: '결제 상태를 확인 중입니다' }
    : { AWAITING_PAYMENT: 'Payment pending', READY_TO_APPLY: 'Payment confirmed', APPLYING: 'Applying your reservation change', COMPLETED: 'Reservation change complete', CANCELLED: 'Payment cancelled', EXPIRED: customerPaymentFailureMessage(locale), RECONCILIATION_REQUIRED: 'Confirming the payment result', REFUND_PENDING: 'Refunding the price difference', UNKNOWN: 'Checking payment status' }
