...
export function BookingCheckoutPage({ locale = 'ko' }: Props) {
  ...
  return <CustomerBookingShell step="checkout" locale={locale} summary={criteriaSummary}>
    <section className="booking-checkout-page" aria-labelledby="booking-checkout-title">
      ...
      {paymentMode !== 'toss-live' && <p className="test-payment-banner" role="status"><ShieldCheck size={18} aria-hidden="true" /> {bookingText(locale, "테스트 환경입니다. 실제로 결제되지 않아요.")}</p>}
      ...
      {reservation && state !== 'EXPIRED' && state !== 'CONFIRMED' && <section className="booking-payment-ready" aria-labelledby="payment-ready-title"><div><p className="section-kicker">PAYMENT</p><h2 id="payment-ready-title">{bookingText(locale, "객실을 확보했습니다")}</h2><p><Clock3 size={18} aria-hidden="true" /> {bookingText(locale, "남은 확보 시간")} <strong>{holdRemaining}</strong></p><span>{bookingText(locale, "예약자 정보는 잠겨 있습니다. 확보 시간 안에 결제를 완료해 주세요.")}</span></div>{paymentMode === 'toss-test' || paymentMode === 'toss-live' || paymentMode === 'fake' ? <button type="button" className="primary" onClick={openPayment} disabled={state === 'PAYMENT_OPEN'}>{state === 'PAYMENT_OPEN' ? bookingText(locale, "토스 결제창을 여는 중…") : paymentMode === 'fake' ? bookingText(locale, "테스트 결제 완료하기") : paymentMode === 'toss-test' ? bookingText(locale, "토스 테스트 결제") : bookingText(locale, "토스 결제")}</button> : <p className="booking-payment-unavailable" role="status">{bookingText(locale, "결제 설정을 확인할 수 없습니다. 확보 시간 안에 운영자에게 문의해 주세요.")}</p>}</section>}
    </section>
