...
import { requestTossCheckout, localizedTossCheckout } from '../lib/toss-payments'
...
export function BookingCheckoutPage({ locale = 'ko' }: Props) {
  ...
  async function openPayment() {
    ...
    try {
      ...
      setError(locale === 'en' ? 'Payment did not complete. Retry before the hold expires or check the server status.' : reason instanceof Error ? reason.message : bookingText(locale, "결제창을 닫았습니다. 확보 시간 안에 다시 시도해 주세요."))
    }
  }
