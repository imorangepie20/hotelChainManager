...
function BookingApp() {
  ...
  function renderPublicHeader(contentOnly: boolean) {
    return <header className="topbar"><a className="brand" href={locale === 'en' ? '/en' : contentOnly ? '/' : '#top'}><span>STAY</span> HANEUL</a><nav className={mobileMenuOpen ? 'open' : ''} aria-label={locale === 'en' ? 'Main navigation' : '주요 메뉴'}>{!contentOnly && <a onClick={closeMenu} href="/#booking">객실</a>}{pageNavigation.map(item => <a key={item.id} onClick={event => { event.preventDefault(); navigateToPath(item.path) }} href={item.path}>{item.label}</a>)}{!contentOnly && <><a onClick={closeMenu} href="#experiences">경험</a><a onClick={closeMenu} href="#offers">오퍼</a><a onClick={closeMenu} href="#booking">예약</a></>}</nav><div className="header-actions"><a href="/reservations" className="manage-link">{locale === 'en' ? 'Booking (Korean)' : '예약 조회'}</a><span className="lang"><a href={locale === 'en' ? pathname.slice(3) || '/' : pathname} lang="ko" aria-current={locale === 'ko' ? 'page' : undefined}>KO</a><span> / </span><a href={locale === 'en' ? pathname : pathname === '/' ? '/en' : '/en' + pathname} lang="en" aria-current={locale === 'en' ? 'page' : undefined}>EN</a></span><button className="menu" aria-label={locale === 'en' ? 'Menu' : '메뉴'} aria-expanded={mobileMenuOpen} onClick={() => setMobileMenuOpen(open => !open)}>{mobileMenuOpen ? <X size={20} /> : <Menu size={20} />}</button></div></header>
  }
