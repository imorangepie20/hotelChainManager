...
legacyStorage.setItem('latestReservation', 'legacy-latest')
legacyStorage.setItem('reservation:legacy-latest', 'old-token')
legacyStorage.setItem('reservation:legacy-earlier', 'earlier-token')
const migratedStore = new BookingSessionStore(legacyStorage)
expectEqual(migratedStore.listReservationAccess(), [{ reservationId: 'legacy-latest', managementToken: 'old-token' }], '병합 전 최근 예약 token을 보존한다')
expectEqual(migratedStore.loadReservationAccess('legacy-earlier'), { reservationId: 'legacy-earlier', managementToken: 'earlier-token' }, '기존 결제 복귀 경로는 이전 예약별 token으로 복구한다')
expectEqual(migratedStore.loadReservationAccess('absent'), null, '저장되지 않은 token을 만들지 않는다')
