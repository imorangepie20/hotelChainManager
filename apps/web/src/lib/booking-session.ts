...
export class BookingSessionStore {
  ...
  loadReservationAccess(reservationId: string): ReservationAccess | null {
    const saved = this.listReservationAccess().find(item => item.reservationId === reservationId)
    if (saved) return saved
    const managementToken = this.get(`reservation:${reservationId}`)
    return managementToken ? { reservationId, managementToken } : null
  }

  listReservationAccess(): ReservationAccess[] {
    const stored = parseStoredValue(this.get(reservationAccessStorageKey))
    const accesses: ReservationAccess[] = isRecord(stored) && stored.version === storageVersion && Array.isArray(stored.accesses) ? stored.accesses.filter(isReservationAccess) : []
    const legacyId = this.get('latestReservation')
    const legacyToken = legacyId ? this.get(`reservation:${legacyId}`) : null
    if (legacyId && legacyToken && !accesses.some(item => item.reservationId === legacyId)) accesses.push({ reservationId: legacyId, managementToken: legacyToken })
    return accesses
  }
