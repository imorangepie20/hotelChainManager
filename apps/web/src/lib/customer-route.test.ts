...
export function runCustomerRouteTests() {
  ...
  expectEqual(resolveCustomerRoute('/contact'), { kind: 'contact', pathname: '/contact' }, '고객 요청 접수 경로를 전용 경로로 구분한다')
  expectEqual(resolveCustomerRoute('/en/contact'), { kind: 'contact', pathname: '/en/contact', locale: 'en' }, '영문 고객 요청 경로를 구분한다')
