...
- 고객 변경 세션은 전체 `reservationId`와 표시용 마지막 8자리를 함께 반환한다. 상세와 추가 결제 재개는 전체 UUID가 정확히 일치할 때만 연결한다.
...
- 공개 영문 헤더의 예약 관리 링크를 `/en/reservations`와 `My reservations`로 수정했다.
- `/en/reservation-change-payment`은 locale을 전달받아 영문 문구와 `en-US` 통화·날짜 형식을 사용한다. 결제 시작·fragment 토큰 교환·HttpOnly 세션·redirect 동작은 유지한다.
- 위 UUID 계약을 반영해 이전 suffix 제한 설명을 제거했다.
