...
type AddableBlock = Exclude<ContentBlock["type"], "HERO">;
...
function blocks(value: unknown): ContentBlock[] {
  return Array.isArray(value) ? value.filter((block): block is ContentBlock => Boolean(block) && typeof block === "object" && !Array.isArray(block) && ["HERO", "TEXT", "CTA", "IMAGE_GALLERY", "FEATURE_GRID", "SPEC_TABLE", "ACCORDION", "NOTICE_LIST", "RICH_TEXT", "OPERATING_HOURS", "LOCATION", "PROMOTION_SUMMARY", "RELATED_COLLECTION", "BOOKING_CTA"].includes(text((block as ContentRecord).type))) : [];
}
...
export function ContentPageEditor({ token, document, catalog, locale = "ko", initialDirty = false, showPublishAction = true, externalBusy = false, previewDisabled, onDirtyChange, onBusyChange, onSaved, onPublished, onLifecycleChanged, onDeleted }: {
  ...
  const addableBlocks: AddableBlock[] = document.contentKind === "ROOM" ? ["IMAGE_GALLERY", "SPEC_TABLE", "BOOKING_CTA", "RICH_TEXT"]
    : document.contentKind === "PROMOTION" ? ["PROMOTION_SUMMARY", "BOOKING_CTA", "RELATED_COLLECTION", "RICH_TEXT"]
      : document.contentKind === "DINING" ? ["IMAGE_GALLERY", "OPERATING_HOURS", "LOCATION", "RICH_TEXT"]
        : document.contentKind === "FACILITY" ? ["IMAGE_GALLERY", "SPEC_TABLE", "LOCATION", "RICH_TEXT"]
          : ["TEXT", "CTA", "IMAGE_GALLERY", "FEATURE_GRID", "SPEC_TABLE", "ACCORDION", "NOTICE_LIST"] as const;
  const selectedRoom = catalog.hotels.flatMap((hotel) => hotel.roomTypes.map((room) => ({ ...room, hotelId: hotel.id, hotelName: hotel.name }))).find((room) => room.id === connections.roomTypeIds[0]);
  const connectionError = document.contentKind === "ROOM" && connections.roomTypeIds.length !== 1
    ? "객실 페이지에는 같은 지점의 객실 유형 하나를 선택해 주세요."
    : document.contentKind === "PROMOTION" && (connections.targetHotelIds.length === 0 || connections.roomTypeIds.some((roomId) => !catalog.hotels.some((hotel) => connections.targetHotelIds.includes(hotel.id) && hotel.roomTypes.some((room) => room.id === roomId))))
      ? "선택한 객실 유형은 대상 지점에 포함되어야 합니다."
      : document.contentKind === "ROOM" && selectedRoom?.hotelId !== document.hotelId ? "선택한 객실 유형은 페이지 지점과 같아야 합니다." : "";
  const change = (next: ContentRecord) => { if (isArchived) return; setContent(next); setDirty(true); setError(""); setNotice(""); };
  const changeConnections = (next: WebsitePageConnections) => { if (isArchived) return; setConnections(next); setDirty(true); setError(""); setNotice(""); };
  ...
  const addBlock = (type: Exclude<ContentBlock["type"], "HERO">) => {
    const defaults: Record<string, ContentBlock> = {
      ...
      RICH_TEXT: { type: "RICH_TEXT", title: "새 소개", paragraphs: ["내용을 입력해 주세요."] },
      OPERATING_HOURS: { type: "OPERATING_HOURS", title: "운영 시간", entries: [{ dayLabel: "매일", opensAt: "09:00", closesAt: "18:00", closed: false }] },
      LOCATION: { type: "LOCATION", title: "오시는 길", address: "주소를 입력해 주세요.", directions: "안내를 입력해 주세요." },
      PROMOTION_SUMMARY: { type: "PROMOTION_SUMMARY", title: "프로모션 안내", salesPeriod: "기간을 입력해 주세요.", stayPeriod: "기간을 입력해 주세요.", benefits: ["혜택을 입력해 주세요."] },
      RELATED_COLLECTION: { type: "RELATED_COLLECTION", title: "함께 보는 콘텐츠", collectionKind: "ROOM" },
      BOOKING_CTA: { type: "BOOKING_CTA", title: "객실 검색", description: "실시간 객실을 확인합니다.", label: "객실 검색", hotelId: document.hotelId ?? "" },
    };
  ...
  async function save() {
    if (isArchived || interactionBusy || connectionError) return;
  ...
  async function publish() {
    if (dirty || isArchived || interactionBusy) return;
    setBusy(true); setError(""); setNotice("");
    try {
  ...
  async function archive() {
    if (!isContentPage || interactionBusy || dirty) return;
    setBusy(true); setError(""); setNotice("");
    try {
  ...
  async function restore() {
    if (!isContentPage || !isArchived || interactionBusy) return;
    setBusy(true); setError(""); setNotice("");
    try {
