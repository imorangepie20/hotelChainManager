...
// 승인 TTL은 초 단위로 저장되지만 본사는 시간 단위로 입력한다.
// 1시간 = 3600초, 최소 1분(60초), 최대 7일(168시간)이다.
...
export function Policies() {
  ...
  async function submitLimit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setLimitError("");
    try {
      await updateChangeApprovalLimit(
        sessionToken,
        `update-limit-${limitUpdateKey}`,
  ...
  async function submitTtl(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!sessionToken) return;
    // 정수 파싱만 클라이언트가 확인하고 범위 판단은 서버에 둔다.
    // 그래야 서버 검증 메시지를 대화상자에서 볼 수 있다.
    ...
    setTtlError("");
    try {
      await updateChangeApprovalTtl(
        sessionToken,
        `update-approval-ttl-${ttlUpdateKey}`,
        {
          approvalTtlSeconds: parsed * 3600,
        },
      );
      setTtlUpdateKey((key) => key + 1);
      setTtlOpen(false);
      await refresh();
    } catch (cause) {
      setTtlError(
  ...
  function openTtlDialog() {
    setTtlError("");
    if (policy) {
      const hours = policy.changeApprovalTtlSeconds / 3600;
      // 시간 단위로 떨어지면 시간을, 아니면 초를 그대로 보여준다.
      setTtlHours(Number.isInteger(hours) ? String(hours) : "24");
    }
    setTtlOpen(true);
  }
  ...
  const revisionLastPage = Math.max(
    0,
    Math.ceil((revisions?.totalCount ?? 0) / REVISION_PAGE_SIZE) - 1,
  );
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 max-w-[72ch] text-sm text-muted-foreground">
            취소 정책과 예약 변경 승인 한도의 현재값을 확인하고 변경한다. 변경은
            신규 예약·변경 요청부터 적용된다.
          </p>
        </div>
        ...
        <div className="grid gap-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                체크인 며칠 전까지 전액 환급되는지, 마감 시각은 언제인지
                보여준다. 변경하면 신규 예약부터 적용된다.
              </CardDescription>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                지점이 직접 승인할 수 있는 차액 한도와 본사 승인 대기 시간, 정산
                활성 여부를 보여준다. 한도·TTL 변경은 진행 중인 변경 요청에
                영향을 주지 않는다.
              </CardDescription>
            </CardHeader>
            <CardContent className="grid gap-3 text-sm">
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  <span
                    className="font-medium tabular-nums"
                    data-testid="approval-ttl"
                  >
                    {ttlHoursLabel(policy.changeApprovalTtlSeconds)}
                  </span>
                  <Button
                    type="button"
                    variant="outline"
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <Badge
                  variant={
                    policy.changeSettlementEnabled ? "default" : "secondary"
                  }
                >
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">
                정책 변경 이력 {revisions?.totalCount ?? 0}건
              </CardTitle>
              <CardDescription>
                취소 정책·예약 변경 승인 한도의 변경을 최신순으로 보여준다. 처리
                직원과 시각을 함께 표시한다.
              </CardDescription>
            </CardHeader>
                ...
                <Table>
                  ...
                  <TableBody>
                    {revisionRows.map((revision) => (
                      <TableRow
                        key={`${revision.key}-${revision.createdAt}-${revision.staffEmail}`}
                      >
                        ...
                        <TableCell>
                          <Badge variant="secondary">
                            {policyRevisionLabels[revision.key] ?? revision.key}
                          </Badge>
      ...
      {isHeadquarters &&
        revisions &&
        revisions.totalCount > REVISION_PAGE_SIZE && (
          <div
            className="flex items-center justify-center gap-2"
            data-testid="revisions-paging"
          >
            <Button
              type="button"
              variant="outline"
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              마감 일수는 1~30, 마감 시각은 HH:MM 형식이다. 이미 확정된 예약의
              취소 조건은 그대로 유지된다.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={submitEdit} className="grid gap-3">
            ...
            <DialogFooter>
              ...
              <Button
                type="submit"
                disabled={saving}
                data-testid="edit-cancellation-submit"
              >
                {saving ? "변경하는 중" : "변경"}
              </Button>
            </DialogFooter>
      ...
      <Dialog open={limitOpen} onOpenChange={setLimitOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              지점이 본사 승인 없이 직접 처리할 수 있는 차액의 최대값이다. 0원
              이상 10,000,000원 이하이며, 진행 중인 예약 변경 요청은 이미 저장된
              한도를 유지한다.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={ttlOpen} onOpenChange={setTtlOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>예약 변경 승인 TTL 변경</DialogTitle>
            <DialogDescription>
              본사 승인을 기다릴 수 있는 최대 시간이다. 1시간(1) 이상
              168시간(7일) 이하이며, 진행 중인 예약 변경 요청은 이미 저장된 만료
              시각을 유지한다.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={submitTtl} className="grid gap-3">
            <label className="grid gap-1 text-sm font-medium">
              ...
              <input
                ...
                data-testid="edit-approval-ttl-value"
              />
            </label>
            {ttlError && (
              <p role="alert" className="break-words text-sm text-destructive">
            ...
            <DialogFooter>
              ...
              <Button
                type="submit"
                disabled={saving}
                data-testid="edit-approval-ttl-submit"
              >
                {saving ? "변경하는 중" : "변경"}
              </Button>
            </DialogFooter>
