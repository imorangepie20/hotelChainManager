...
test("reports the current cancellation policy and change limits", async ({ page }) => {
  await page.addInitScript(seedStaffScript("HQ_ADMIN"));
...
test("keeps the dialog open with a server validation message", async ({ page }) => {
  ...
  await page.route("**/api/staff/policies/cancellation", (route) =>
    route.fulfill({
      status: 400,
      contentType: "application/json",
      body: JSON.stringify({ code: "INVALID_REQUEST", message: "취소 마감 시각은 HH:MM 형식이어야 합니다." }),
    }),
  );
  ...
  await expect(page.getByText("취소 마감 시각은 HH:MM 형식이어야 합니다.")).toBeVisible();
...
test("tells a branch employee the view is headquarters only", async ({ page }) => {
  ...
  await expect(page.getByText("공통 정책은 본사 관리자만 확인할 수 있습니다.")).toBeVisible();
...
test("shows a recovery notice when the policy is unreachable", async ({ page }) => {
  await page.addInitScript(seedStaffScript("HQ_ADMIN"));
  await page.route(/.*\/api\/staff\/policies(\?.*)?$/, (route) => route.fulfill({ status: 500 }));

  await page.goto("/dashboard/policies");

  await expect(page.getByText("공통 정책을 불러오지 못했습니다.")).toBeVisible();
});
...
test("updates the change approval limit and reflects it", async ({ page }) => {
  ...
  await page.route("**/api/staff/policies/change-limit", (route) => {
    ...
    return route.fulfill({
      status: 201,
      contentType: "application/json",
      body: JSON.stringify({ directLimitKrw: 300000, revision: 1, created: true }),
    });
  });
...
test("keeps the limit dialog open with a server validation message", async ({ page }) => {
  await page.addInitScript(seedStaffScript("HQ_ADMIN"));
  ...
  await expect(page.getByText("예약 변경 승인 한도는 0원 이상 10,000,000원 이하여야 합니다.")).toBeVisible();
...
test("reports policy revisions with staff context", async ({ page }) => {
  ...
  await expect(page.getByRole("cell", { name: "예약 변경 승인 한도 300,000원" })).toBeVisible();
  await expect(page.getByRole("cell", { name: "취소 정책 체크인 3일 전 20:30 마감" })).toBeVisible();
...
test("moves to the next revision page", async ({ page }) => {
  await page.addInitScript(seedStaffScript("HQ_ADMIN"));
  let offset = 0;
  await page.route(/.*\/api\/staff\/policies\/revisions(\?.*)?$/, async (route) => {
  ...
  await expect(page.getByRole("cell", { name: "예약 변경 승인 한도 200,000원" })).toBeVisible();
  await expect(page.getByRole("button", { name: "다음 페이지" })).toBeDisabled();
});

test("tells a branch employee the revisions are headquarters only", async ({ page }) => {
  await page.addInitScript(seedStaffScript("BRANCH_STAFF"));
  await page.goto("/dashboard/policies");
